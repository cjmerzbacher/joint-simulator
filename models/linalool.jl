...
function linalool(du, u, p, t)
    lam, v_in, W = p
    hmgcoa, ipp, dmapp, gpp, fpp, linalool, thmgr, idi, erg20, lis = u
    k_a, k_b, k_c, k_d = W #[RPU]

    alpha = lnl_params("alpha")

    v_mev = thmgr * michaelismenten(hmgcoa, lnl_params("kcat_thmgr_hmgcoa"), lnl_params("km_thmgr_hmgcoa"))
    ...
    du[1] = v_in - lam*hmgcoa #hmgcoa #changed from lit model where assumed constant from native metabolism, now from FBA
    du[2] = v_mev + v_idi_dmapp - v_idi_ipp - v_erg20_dmapp - v_erg20_gpp - lam*ipp #ipp
    du[3] = v_idi_ipp - v_idi_dmapp - v_erg20_dmapp - lam*dmapp #dmapp
    du[4] = v_erg20_dmapp - v_erg20_gpp - v_lis_gpp - lam*gpp #gpp
    du[5] = v_erg20_gpp - lam*fpp #fpp
    du[6] = v_lis_gpp - lam*linalool #linalool
    du[7] = alpha*k_a - lam*thmgr #thmgr
    du[8] = alpha*k_b - lam*idi #idi
    du[9] = alpha*k_c - lam*erg20 #erg20
    du[10] = alpha*k_d - lam*lis #lis
end